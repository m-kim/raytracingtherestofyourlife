...
public:
  ...
  public:
                              ...
                              const vtkm::Id& idx) const
    {
      ...
      colors.Set(offset + 0, normal[0]);
      colors.Set(offset + 1, normal[1]);
      colors.Set(offset + 2, normal[2]);
...
RayTracerNormals::RayTracerNormals()
  : NumberOfShapes(0)
  , Shade(true)
{
}
...
Camera& RayTracerNormals::GetCamera()
{
  return camera;
}


void RayTracerNormals::AddShapeIntersector(ShapeIntersector* intersector)
{
  NumberOfShapes += intersector->GetNumberOfShapes();
  Intersectors.push_back(intersector);
}

void RayTracerNormals::SetField(const vtkm::cont::Field& scalarField, const vtkm::Range& scalarRange)
{
  ScalarField = scalarField;
  ScalarRange = scalarRange;
}

void RayTracerNormals::SetColorMap(const vtkm::cont::ArrayHandle<vtkm::Vec<vtkm::Float32, 4>>& colorMap)
{
  ColorMap = colorMap;
}

void RayTracerNormals::Render(Ray<vtkm::Float32>& rays)
{
  RenderOnDevice(rays);
}

void RayTracerNormals::Render(Ray<vtkm::Float64>& rays)
{
  RenderOnDevice(rays);
}

void RayTracerNormals::SetShadingOn(bool on)
{
  Shade = on;
}

vtkm::Id RayTracerNormals::GetNumberOfShapes() const
{
  return NumberOfShapes;
}

void RayTracerNormals::Clear()
{
...
template <typename Precision>
void RayTracerNormals::RenderOnDevice(Ray<Precision>& rays)
{
