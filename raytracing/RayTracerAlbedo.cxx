...
public:
  ...
  public:
                              ...
                              const vtkm::Id& idx) const
    {
      ...
      vtkm::Vec<Precision, 3> absorbed;
      absorbed[0] = (1. - reflect[0]/lightDir[0]) * reflect[0];
      absorbed[1] = (1. - reflect[1]/lightDir[1]) * reflect[1];
      absorbed[2] = (1. - reflect[2]/lightDir[2]) * reflect[2];
      colors.Set(offset + 0, (cosPhi*reflect[0])/(cosTheta * lightDir[0]));
      colors.Set(offset + 1, (cosPhi * reflect[1])/(cosTheta * lightDir[1]));
      colors.Set(offset + 2, (cosPhi * reflect[2])/(cosTheta * lightDir[2]));
...
RayTracerAlbedo::RayTracerAlbedo()
  : NumberOfShapes(0)
  , Shade(true)
{
}
...
Camera& RayTracerAlbedo::GetCamera()
{
  return camera;
}


void RayTracerAlbedo::AddShapeIntersector(ShapeIntersector* intersector)
{
...
void RayTracerAlbedo::SetColorMap(const vtkm::cont::ArrayHandle<vtkm::Vec<vtkm::Float32, 4>>& colorMap)
{
  ColorMap = colorMap;
}

void RayTracerAlbedo::Render(Ray<vtkm::Float32>& rays)
{
  RenderOnDevice(rays);
}

void RayTracerAlbedo::Render(Ray<vtkm::Float64>& rays)
{
  RenderOnDevice(rays);
}

void RayTracerAlbedo::SetShadingOn(bool on)
{
  Shade = on;
}

vtkm::Id RayTracerAlbedo::GetNumberOfShapes() const
{
  return NumberOfShapes;
}

void RayTracerAlbedo::Clear()
{
